-- Talla de polera y short, para la compra/entrega de uniforme.
-- EJECUCIÓN MANUAL: Supabase Dashboard > SQL Editor.

BEGIN;

ALTER TABLE jugadores
  ADD COLUMN IF NOT EXISTS talla_polera text,
  ADD COLUMN IF NOT EXISTS talla_short  text;

ALTER TABLE solicitudes_jugador
  ADD COLUMN IF NOT EXISTS talla_polera text,
  ADD COLUMN IF NOT EXISTS talla_short  text;

-- Misma función de 069, agregando talla_polera/talla_short al final de la
-- firma (DEFAULT NULL, no rompe llamadas existentes) y al INSERT.
...
REVOKE ALL ON FUNCTION public.crear_solicitud_jugador(text,uuid,text,text,text,text,text,text,text,text,text,text,text,text,text,text,text,text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.crear_solicitud_jugador(text,uuid,text,text,text,text,text,text,text,text,text,text,text,text,text,text,text,text) TO anon, authenticated;

-- La firma anterior (16 parámetros, de la migración 069) queda huérfana: se
-- elimina para que PostgREST no tenga dos candidatas y falle por ambigüedad.
DROP FUNCTION IF EXISTS public.crear_solicitud_jugador(text,uuid,text,text,text,text,text,text,text,text,text,text,text,text,text,text);
