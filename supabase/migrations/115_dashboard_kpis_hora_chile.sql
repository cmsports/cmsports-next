-- ────────────────────────────────────────────────────────────
-- dashboard_kpis usaba `current_date`, que es la fecha del SERVIDOR
-- (Postgres corre en UTC en Supabase), no la de Chile. Como Chile va
-- atrasado respecto a UTC, cualquier noche —y sobre todo la noche del
-- último día del mes— el servidor ya cree que es el día siguiente, o el
-- mes siguiente. `dashboard_kpis` filtraba "movimientos de este mes"
-- contra ese mes equivocado: si ya no hay movimientos cargados ahí,
-- ingresos y gastos del dashboard se ven en $0 aunque sí existan.
--
-- El resto del proyecto ya resuelve esto con
-- `(now() AT TIME ZONE 'America/Santiago')::date` en más de diez
-- funciones (032, 040, 042, 087, 092, 098...). Esta es la única que había
-- quedado con `current_date` pelado. Mismo cuerpo que la versión de la
-- migración 018, cambiando solo el origen de "hoy".
--
...
grant execute on function dashboard_kpis(uuid) to authenticated;
