-- Punto 7 de la reunión: antes de aprobar, el club puede devolverle la
-- solicitud al postulante con observaciones para que corrija sus datos.
--
-- Flujo: admin escribe la observación -> estado pasa a 'correccion' -> se le
-- avisa por WhatsApp con el mismo link -> el postulante vuelve a completar el
-- formulario -> la solicitud se actualiza y vuelve a 'pendiente'.
...
-- Reenvío del formulario: si la solicitud está esperando corrección, se
-- actualiza con los datos nuevos y vuelve a la cola de pendientes.
...
BEGIN
  ...
  -- Reenvío tras corrección: se actualiza la solicitud existente y vuelve a
  -- pendiente. No consume cuota de identidad: el club le pidió corregir.
  SELECT s.id INTO v_id
  FROM public.solicitudes_jugador s
  WHERE s.club_id = p_club_id AND s.estado = 'correccion'
    AND (lower(COALESCE(s.email, '')) = v_email OR COALESCE(s.rut, '') = v_rut)
  ORDER BY s.creado_en DESC LIMIT 1;

  IF v_id IS NOT NULL THEN
    UPDATE public.solicitudes_jugador SET
      nombre    = v_nombre,
      rut       = v_rut,
      email     = v_email,
      telefono  = v_telefono,
      estado    = 'pendiente',
      fecha_nacimiento             = NULLIF(trim(COALESCE(p_fecha_nacimiento, '')), ''),
      direccion                    = NULLIF(trim(COALESCE(p_direccion, '')), ''),
      comuna                       = NULLIF(trim(COALESCE(p_comuna, '')), ''),
      contacto_emergencia_nombre   = NULLIF(trim(COALESCE(p_contacto_emergencia_nombre, '')), ''),
      contacto_emergencia_telefono = NULLIF(trim(COALESCE(p_contacto_emergencia_telefono, '')), ''),
      indicaciones_medicas         = NULLIF(trim(COALESCE(p_indicaciones_medicas, '')), ''),
      nombres    = NULLIF(trim(COALESCE(p_nombres, '')), ''),
      apellido1  = NULLIF(trim(COALESCE(p_apellido1, '')), ''),
      apellido2  = NULLIF(trim(COALESCE(p_apellido2, '')), ''),
      apellido3  = NULLIF(trim(COALESCE(p_apellido3, '')), ''),
      corregida_en    = now(),
      veces_corregida = veces_corregida + 1
    WHERE id = v_id;
    RETURN v_id;
  END IF;
