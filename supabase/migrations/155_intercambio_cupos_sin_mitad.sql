-- Quita la restricción de "misma mitad del cuadro" al intercambiar cupos
-- en la ronda inicial. Se puede mover un jugador de cualquier llave a
-- cualquier otra (dentro de la ronda inicial no jugada).
-- Se mantiene: admin, ronda inicial, llave no jugada de verdad, no dejar
-- llave vacía, y que un grupo no se enfrente a sí mismo.
--
-- EJECUCIÓN MANUAL: Supabase Dashboard > SQL Editor.

BEGIN;
SELECT _migracion_nueva('155_intercambio_cupos_sin_mitad');
...
begin
  ...
  -- Sin chequeo de mitades: se puede intercambiar entre cualquier llave
  -- de la ronda inicial (no jugada).
