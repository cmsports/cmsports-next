-- El esqueleto del bracket ahora se arma apenas hay grupos (ver
-- sincronizarLlaves en torneos.ts), en vez de esperar la mitad de los
-- grupos cerrados. Bloquear el cambio de cabezas de serie por la sola
-- existencia de partidos de bracket dejaba la edición inutilizable casi
-- desde el inicio del torneo. Se relaja a: bloquear solo si ya se jugó
-- una llave real (mismo criterio que llaveFueJugada en el backend:
-- ganador y jugador_b definidos), momento en que sí importa no mover
-- cabezas bajo un árbol ya en juego.
