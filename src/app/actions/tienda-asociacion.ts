...
type TiendaAsociacionUpdate = Database['public']['Tables']['tienda_asociacion_productos']['Update']
...
async function subirImagen(admin: ReturnType<typeof createAdminClient>, clubId: string, id: string, base64: string) {
  ...
  const path = `tienda-asociacion/${clubId}/${id}`
...
export async function crearProductoTiendaAsociacion(params: {
  nombre: string
  descripcion: string
  categoria: string
  color: string
  ...
  if (params.base64) {
    ...
    if (url) await admin.from('tienda_asociacion_productos').update({ imagen_url: url }).eq('id', prod.id)
  }

  return { success: true }
}

export async function editarProductoTiendaAsociacion(params: {
  id: string
  nombre: string
  descripcion: string
  categoria: string
  ...
  const updates: TiendaAsociacionUpdate = {
    nombre: params.nombre.trim(),
    descripcion: params.descripcion?.trim() || null,
    categoria: params.categoria,
    color: params.color?.trim() || null,
  ...
  await admin.from('tienda_asociacion_productos').update(updates).eq('id', params.id).eq('club_id', clubId)
  return { success: true }
}

export async function eliminarProductoTiendaAsociacion(params: { id: string }) {
  const { error, admin, clubId } = await requireProfesor()
  if (error || !admin || !clubId) return { error }

  await admin.storage.from(BUCKET).remove([`tienda-asociacion/${clubId}/${params.id}`])
  await admin.from('tienda_asociacion_productos').delete().eq('id', params.id).eq('club_id', clubId)
  return { success: true }
}
