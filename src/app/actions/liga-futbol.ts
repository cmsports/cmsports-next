...
export async function generarFixtureLiga(ligaId: string) {
  ...
  if (fechaIds.length > 0) {
    ...
    await supabase.from('lf_partidos').delete().in('fecha_id', fechaIds)
    await supabase.from('lf_fechas').delete().in('id', fechaIds)
  }
  ...
  await supabase.from('lf_ligas').update({ estado: 'en_curso' }).eq('id', ligaId).eq('estado', 'inscripcion')
...
export async function registrarResultado(partidoId: string, golesLocal: number, golesVisita: number) {
  ...
  const { error } = await supabase.from('lf_partidos')
    ...
    .eq('id', partidoId)
  if (error) return { error: error.message }
  await avanzarPlayoffSiCorresponde(supabase, partidoId)
  return { error: null }
}

async function recalcularMarcadorPartido(supabase: any, partidoId: string, equipoLocalId: string, equipoVisitaId: string) {
  ...
  await supabase.from('lf_partidos').update({ goles_local: golesLocal, goles_visita: golesVisita }).eq('id', partidoId)
}
...
export async function registrarGol(params: {
  ...
  await recalcularMarcadorPartido(supabase, params.partido_id, partido.equipo_local_id, partido.equipo_visita_id)
  if (partido.estado === 'programado') {
    await supabase.from('lf_partidos').update({ estado: 'en_curso' }).eq('id', params.partido_id)
  }
...
export async function eliminarGol(golId: string) {
  ...
  await recalcularMarcadorPartido(supabase, gol.partido_id, gol.lf_partidos.equipo_local_id, gol.lf_partidos.equipo_visita_id)
  return { error: null }
}
...
async function crearSancionAutomatica(supabase: any, params: {
  ...
  await supabase.from('lf_sanciones').insert({
...
export async function registrarTarjeta(params: {
  ...
  if (params.tipo === 'roja' || params.tipo === 'doble_amarilla') {
    await crearSancionAutomatica(supabase, {
...
async function avanzarPlayoffSiCorresponde(supabase: any, partidoId: string) {
  const { data: partido } = await supabase.from('lf_partidos')
    .select('liga_id, lf_fechas!inner(es_playoff, fase_playoff)').eq('id', partidoId).single()
  if (!partido?.lf_fechas?.es_playoff) return
  ...
  const { data: partidosFase } = await supabase.from('lf_partidos')
    ...
    .order('orden_bracket')
  if (!partidosFase || partidosFase.length === 0) return
  if (!partidosFase.every((p: any) => p.estado === 'finalizado' || p.estado === 'wo')) return
  ...
  for (const p of partidosFase) {
    ...
    if (!g) return // empate sin resolver — el bracket espera a que el admin corrija el resultado
    ganadores.push(g)
  ...
  if (!siguienteFase) {
    if (fase === 'final') await supabase.from('lf_ligas').update({ estado: 'finalizada' }).eq('id', partido.liga_id)
    return
  }
  ...
  const { data: fechaCreada } = await supabase.from('lf_fechas').insert({
  ...
  if (fechaCreada) {
    await supabase.from('lf_partidos').insert(partidosSiguienteRonda.map(p => ({
  ...
  if (fase === 'semifinal') {
    ...
    if (liga?.tercer_lugar) {
      ...
      if (partidoTercerLugar.length > 0) {
        const { data: fechaTercer } = await supabase.from('lf_fechas').insert({
          liga_id: partido.liga_id, numero: siguienteNumero, nombre: labelFase('tercer_lugar'),
          es_playoff: true, fase_playoff: 'tercer_lugar',
        }).select('id').single()
        if (fechaTercer) {
          await supabase.from('lf_partidos').insert(partidoTercerLugar.map(p => ({
...
export async function iniciarPlayoffs(ligaId: string) {
  ...
  await supabase.from('lf_ligas').update({ estado: 'playoffs' }).eq('id', ligaId)
  return { error: null }
}
